...
- **WAL 持久化**: 组提交路径增加预滚动与析构 flush，避免崩溃时丢单并强制遵守 1GB 切卷。
- **结算强平可靠性**: 强平线程自动启动，队列满时不再静默丢任务，异步强平引用更安全。
- **因子计算**: 并行 DAG 每次执行强制重算节点，避免缓存导致的指标陈旧。
- **行情广播**: 慢订阅者检测阈值与配置保持一致，批量广播逻辑可调。
- **前端**: 权益曲线 CSV 导出修复，生产构建已通过。
