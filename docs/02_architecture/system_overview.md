...
**核心组件**:
- **增量算子** (`operators/`): RollingMean, EMA, RSI, MACD 等 O(1) 算子
- **环形缓冲区** (`ring_buffer.rs`): 滑动窗口高效实现
- **Welford 算法** (`welford.rs`): 数值稳定的在线统计
- **因子 DAG** (`dag.rs`): 依赖管理与并行计算
- **物化视图** (`view.rs`): 因子状态持久化
